...
<div class="warning">
<h2>{"Are you sure you want to set metatags attributes for these classes?"|i18n("design/standard/class/edit")}</h2>
...
{section name=Result loop=$DeleteResult}
	<li>{"Setting metatags attributes for class %1 will add the attribute to %2 objects!"|i18n("design/standard/class/edit",,array($Result:item.className|wash,$Result:item.objectCount))}</li>
{/section}
</ul>
</div>
<form action={concat($module.functions.setclassmetatags.uri,"/",$GroupID,"/",$ClassID)|ezurl} method="post" name="ClassRemove">
